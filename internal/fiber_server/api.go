...
func (server *Server) Start(port string) {
	// start server
	log.Fatal(server.app.Listen(port), "Start server error")
}

func (server *Server) StartHttp2(_ string) {
	panic("HTTP 2.0 does not implemented")
	...
	//
	//// start app
	//log.Fatal(server.app.Listen(port), "Start HTTP2 server error")
}
...
func (server *Server) ActivateSwagger(customEndpoint ...string) {
	...
	server.app.Get(endpoint, swagger.HandlerDefault)
}

func (server *Server) Get() *fiber.App {
	return server.app
}
